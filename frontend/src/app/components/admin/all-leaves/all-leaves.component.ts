...
@Component({
  ...
  template: `
    <div class="all-leaves-container">
      <!-- Header -->
      <div class="page-header">
        ...
        <p>Manage and review all employee leave requests</p>
      </div>
      ...
      <div class="controls-section">
        <div class="filters">
          ...
          <div class="filter-group">
            ...
            <select id="employeeFilter" [(ngModel)]="filters.employee" (change)="applyFilters()">
              <option value="">All Employees</option>
              <option *ngFor="let employee of uniqueEmployees" [value]="employee.userId">
                {{employee.userName}}
              </option>
            </select>
          </div>
          ...
          <div class="filter-group">
            ...
            <select id="monthFilter" [(ngModel)]="filters.month" (change)="applyFilters()">
              <option value="">All Months</option>
              <option *ngFor="let month of availableMonths; let i = index" [value]="i">
                {{month}}
              </option>
            </select>
          </div>
      ...
      <div class="table-container">
        <div class="table-header">
          <h2>Leave Requests ({{filteredLeaves.length}})</h2>
        ...
        <div class="empty-state" *ngIf="!isLoading && filteredLeaves.length === 0">
          ...
          <p *ngIf="!hasActiveFilters()">No leave requests have been submitted yet.</p>
              ...
              <tr *ngFor="let leave of filteredLeaves" class="leave-row">
                <td>
                  <div class="employee-info">
                    <div class="employee-name">{{leave.userName || 'Unknown'}}</div>
                    <div class="employee-email">{{leave.userEmail}}</div>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <div class="date-info">
                    ...
                    <div class="created-date">Submitted: {{formatDate(leave.createdAt)}}</div>
                  </div>
                </td>
                <td>
                  ...
                  <div class="action-buttons">
                    <button 
                      *ngIf="leave.status === 'PENDING'"
                      ...
                      title="Approve">
                      <span *ngIf="!processingLeaves.has(leave.id)">✅</span>
                      <span *ngIf="processingLeaves.has(leave.id)" class="spinner-small"></span>
                    </button>
                    <button 
                      *ngIf="leave.status === 'PENDING'"
                      class="btn-reject"
        ...
        <div class="modal-content" (click)="$event.stopPropagation()">
          ...
          <div class="modal-body">
            <div class="detail-row">
              ...
              <div>
                <div>{{selectedLeave.userName || 'Unknown'}}</div>
                <div class="detail-secondary">{{selectedLeave.userEmail}}</div>
              </div>
            </div>
            <div class="detail-row">
              <strong>Type:</strong>
          ...
          <!-- Modal Actions -->
          <div class="modal-actions" *ngIf="selectedLeave.status === 'PENDING'">
            <button 
              class="btn-approve-modal"
              (click)="approveLeave(selectedLeave)"
              [disabled]="processingLeaves.has(selectedLeave.id)">
              <span *ngIf="!processingLeaves.has(selectedLeave.id)">✅ Approve</span>
              <span *ngIf="processingLeaves.has(selectedLeave.id)" class="processing">
            ...
            <button 
              ...
              [disabled]="processingLeaves.has(selectedLeave.id)">
              <span *ngIf="!processingLeaves.has(selectedLeave.id)">❌ Reject</span>
              <span *ngIf="processingLeaves.has(selectedLeave.id)" class="processing">
  ...
  `,
  styles: [`
    /* Base styles similar to leave-history component but with admin-specific colors and layout */
    ...
    .btn-approve-modal:hover:not(:disabled) {
      background: #059669;
      border-color: #059669;
    }
    ...
    .btn-reject-modal:hover:not(:disabled) {
      background: #dc2626;
      border-color: #dc2626;
    }

    .btn-approve-modal:disabled, .btn-reject-modal:disabled {
      opacity: 0.6;
      cursor: not-allowed;
    }
...
export class AllLeavesComponent implements OnInit {
  ...
  uniqueEmployees: { userId: number; userName: string }[] = [];
  ...
  generateUniqueEmployees() {
    const employeeMap = new Map<number, string>();
    this.allLeaves.forEach(leave => {
      if (leave.userId && leave.userName) {
        employeeMap.set(leave.userId, leave.userName);
      }
    });
    
    this.uniqueEmployees = Array.from(employeeMap.entries())
  ...
  applyFilters() {
    ...
    if (this.filters.employee) {
      const employeeId = parseInt(this.filters.employee);
      filtered = filtered.filter(leave => leave.userId === employeeId);
    }
  ...
  hasActiveFilters(): boolean {
    return !!(this.filters.status || this.filters.type || this.filters.employee || this.filters.month);
  }
  ...
  private processLeaveAction(leave: LeaveResponse, status: string, comments?: string) {
    ...
    this.apiService.approveLeave(leave.id, request).subscribe({
      next: (response) => {
        ...
        if (response.success && response.data) {
          // Update the leave in our arrays
          const updatedLeave = response.data;
          const index = this.allLeaves.findIndex(l => l.id === leave.id);
          if (index !== -1) {
            this.allLeaves[index] = updatedLeave;
          }
          
          // Update filtered leaves
          const filteredIndex = this.filteredLeaves.findIndex(l => l.id === leave.id);
          if (filteredIndex !== -1) {
            this.filteredLeaves[filteredIndex] = updatedLeave;
          }

          // Update selected leave if it's the same one
          if (this.selectedLeave?.id === leave.id) {
            this.selectedLeave = updatedLeave;
          }
